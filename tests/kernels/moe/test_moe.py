...
def test_canonicalize_marlin_moe_token_order():
    ...
    expert_ids = torch.tensor([2, 2, 7, 7, -123], dtype=torch.int32)
    ...
    torch.testing.assert_close(
        first_canonical[:12], expected_active, atol=0, rtol=0
    )
    torch.testing.assert_close(
        second_canonical[:12], expected_active, atol=0, rtol=0
    )
